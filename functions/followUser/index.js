...
module.exports = function (context, req) {
    ...
    if (req.body.username) {
        let entity = {
            ...
            RowKey: entGen.String(`${req.body.username}-${req.body.followed}`),
            Timestamp: entGen.DateTime(new Date(Date.UTC(2011, 10, 25))),
            user: entGen.String(req.body.username),
            follows: entGen.String(req.body.followed)
        };

        const insertData = () => new Promise((resolve, reject) => {
            tableService.insertEntity('Followers', entity, function(error, result, response) {
                if (error) {
                    return reject(error)
                }
                resolve(result)
